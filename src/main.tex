...
\vspace*{-6mm}  % Adjusts the vertical space, move content up by 10mm
  ...
  \profilesection{Teaching, Talks }
		...
		\skill{\faChalkboardTeacher}{WebRTC (Intl dev conf, Berlin)}

	\profilesection{Short Bio}
	\aboutme{
    With over 26 years of experience in web development across digital TV, media platforms, and mobile devices, I have led the architecture, development, and deployment of numerous projects. My leadership role included enhancing our software stack by creating modules, conducting code reviews, and implementing agile practices. I am committed to developing user-friendly and scalable web solutions using technologies like AWS, Angular, Vue.js, React, PHP, Python, HTML5, and AI.}\\
}
%-------------------------------------------------------------------------------
...
\begin{cvtable}
    ...
    {Cisco, Paris}
    {
      \textbf{Project}: Development of an HTML5 app for Belgacom on RDK middleware.\\
      \textbf{Responsibilities}: Developed interactive features using CSS3D for GPU hardware acceleration, and managed data interactions using vanilla WebSockets and AJAX within a BackboneJS framework.
       Integrated complex WebGL content managed by Three.JS according to COLLADA standards for 3D object models, enhanced with animations via Tween.JS.
       Adapted the application for tablet platforms like iPad (iOS8) and Nexus 10 (Android) using Hammer.JS for touch event handling.

      \textbf{Challenges}: Packaged the WebApp using cross-platform tools like Adobe PhoneGap and CocoonJS, addressing performance optimizations and compatibility issues on low-end STBs without WebGL support.
       Explored various rendering options (Three.JS CanvasRenderer, CSSRenderer, Parallax scrolling) to maintain performance and visual quality.

      \textbf{Outcome}: Successfully delivered a proof of concept that balanced performance with hardware acceleration, leading the competition.
    }
\end{cvtable}
...
\begin{cvtable}
    ...
    {France}
    {
      \textbf{Project}: Development of HTML5, CSS3D applications for embedded devices (smart TV, Raspberry Pi), showcased at CES 2014.\\
      \textbf{Responsibilities}:
          Implemented application features across multiple devices including LG TVs, HDMI sticks, tablets, and laptops using HTML, JS, CSS3D, and WebGL.\\
          Utilized BackboneJS for MVC framework, ThreeJS for 3D modeling, and TweenJS for animations.\\
          Enhanced performance with hardware acceleration assessments across various HTML5 engines like QT5/Webkit1, WebKitNix, and Chromium.\\
          Managed cross-compilation efforts for platforms including LG TV, Raspberry Pi, Cotton Candy (FXI), and MacOSX.\\
          Compiled and built GStreamer for Windows and MacOSX, ensuring compatibility and performance.\\
      \textbf{Outcome}: Achieved high performance and compatibility across all targeted devices, contributing to a successful presentation at CES and further engagements with Belgacom and Zon TV.\\
    }
\end{cvtable}
