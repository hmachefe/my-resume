...
\begin{cvtableNew}
  ...
  \cvitemRightNew
      ...
      {difuze inc} % Company
      {
        ...
        \begin{itemize}[itemsep=-1mm, topsep=0pt, leftmargin=8pt]
          \item \textbf{difuzego.com:} MPEG-DASH encoding (AWS MediaConvert→Hybrik), HLS packaging (EC2 macOS→FFmpeg) with AES-128 encryption; integrated Bitmovin, Video.js \& Accurate Player with CanvasJS/Bandit analytics; orchestrated compressed-proxy generation via AWS Step Functions; enhanced on-prem Node.js/Express + SQLite workflows; upgraded to Angular 14 with MFA auth; migrated Elasticsearch → PostgreSQL (Knex-compliant models).\vspace{5pt}
